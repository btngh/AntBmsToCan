...
#include <ESP8266WebServer.h>        // Thư viện tạo Web Server cho Web OTA
#include <ESP8266HTTPUpdateServer.h>   // Thư viện xử lý giao diện nạp code qua Web
...
// ==========================================
// CẤU HÌNH WIFI VÀ MQTT BROKER (GIỮ NGUYÊN CỦA BẠN)
// ==========================================
...
// ==========================================
// SƠ ĐỒ CHÂN PHẦN CỨNG ESP8266 (GIỮ NGUYÊN CỦA BẠN)
// ==========================================
...
// Khởi tạo Web Server chạy cổng 80 và trình nạp code qua mạng
...
void sendMqttData() {
  ...
  // Đóng gói dữ liệu chuỗi JSON để gửi lên Home Assistant hoặc Broker phần mềm
...
bool sendCanMessage()
{
  ...
  byte canResult;
  unsigned long id;
  byte ext = 1; // Khung mở rộng 29-bit mở dải áp cao hệ Solintek HV_1

  // --- Khối 1: ID 0x7310 + Addr (Bắt tay thông số cấu hình hệ thống phần cứng) ---
  if (result) {
    delay(2);
  ...
  // --- Khối 2: ID 0x7320 + Addr (Bắt tay khai báo dung lượng pin 100AH) ---
  if (result) {
    delay(2);
  ...
  // --- Khối 3 & 4: ID 0x7330 & 0x7340 (Khai báo tên thương hiệu ASCII) ---
  if (result) {
    delay(2);
  ...
  // --- Khối 5: ID 0x4210 + Addr (Điện áp tổng HV, Dòng điện kèm bù Offset -3000A, SoC, SoH) ---
  if (result) {
    delay(2);
    id = 0x00004210 + CAN_ADDR; 
    
    // Quy đổi điện áp thực sang dạng 0.1V hệ HV
    uint16_t totalVolt_01V = (uint16_t)(_receivedResponse.totalVoltage * 10.0); 
    
    // Thuật toán áp cao dịch dải dòng điện: (Dòng thực tế + 3000) * 10
  ...
  // --- Khối 6: ID 0x4220 + Addr (Điện áp cắt và Giới hạn bảo vệ dòng sạc xả cực đại) ---
  if (result) {
    delay(2);
  ...
  // --- Khối 7: ID 0x4230 + Addr (Đồng bộ điện áp đơn thể cực trị Cell pin từ AntBMS) ---
  if (result) {
    delay(2);
  ...
  // --- Khối 8: ID 0x4240 + Addr (Đồng bộ nhiệt độ cực trị Cell pin cảm biến thực tế) ---
  if (result) {
    delay(2);
  ...
  // --- CÁC KHỐI PHỤ TRỐNG LẤP ĐẦY TRÁNH LỖI NGẮT KẾT NỐI BIẾN TẦN ---
  memset(data, 0, 8);
