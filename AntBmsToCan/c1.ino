...
// Kích hoạt tính năng báo cáo qua WiFi và MQTT
...
// ĐỊNH NGHĨA CÁC ENUM VÀ STRUCT CỦA SOLXPOW ĐỂ PHỤC VỤ MAPPING CAN
...
void build_frame_4210(uint8_t data[8]) {
  uint16_t totalV = clamp16((int32_t)datalayer.battery.voltage_dV);
  write16(&data[0], totalV);
  int32_t cur_raw = (int32_t)datalayer.battery.reported_current_dA;
  write16(&data[2], (uint16_t)((int16_t)cur_raw));
  int32_t temp_scaled = (int32_t)(datalayer.battery.temperature_max_dC + 1000);
  write16(&data[4], clamp16(temp_scaled));
  uint8_t soc_byte = (uint8_t)constrain((int)round(datalayer.battery.reported_soc / 100.0f), 0, 100);
  uint8_t soh_byte = (uint8_t)constrain((int)round(datalayer.battery.soh_pptt / 100.0f), 0, 100);
  data[6] = soc_byte;
  data[7] = soh_byte;
}
...
void emptyPayload() {
  if (_mqttPayload != NULL) memset(_mqttPayload, 0, _maxPayloadSize);
}

void mqttCallback(char* topic, byte* payload, unsigned int length) {}

void sendMqttState(int errType, int dummy, unsigned long id, byte canRes, int txErr) {
  if (!_mqtt.connected()) return;
  char buf[64];
  snprintf(buf, sizeof(buf), "{\"err\":%d,\"id\":%lu,\"res\":%d,\"tx\":%d}", errType, id, canRes, txErr);
  _mqtt.publish("tele/AntBmsToCan/STATE", buf);
}
...
void printCanResultToSerialMCP(unsigned long id, byte canResult) {
  Serial.printf("CAN Packet 0x%08lX, Result: %d\n", id, canResult);
}
...
bool sendCanMessage() {
  ...
  // Gửi khung dữ liệu cấu hình hệ thống 0x7320
  ...
  // Gửi khung dữ liệu điện áp tổng, dòng điện, SoC 0x4210
  ...
  // Gửi khung dữ liệu chặn dòng sạc/xả 0x4220
  ...
  // Gửi khung dữ liệu cell Max/Min 0x4230
  ...
  // Gửi khung dữ liệu nhiệt độ Max/Min 0x4240
  ...
  // Gửi khung dữ liệu trạng thái bảo vệ lỗi hệ thống 0x4250
...
void readBms() {
  ...
  if (goodCrc) {
    ...
    // THỰC HIỆN ĐỒNG BỘ SANG BIẾN SOLXPOW ĐỂ PHÁT QUA KHUNG CAN BUS
